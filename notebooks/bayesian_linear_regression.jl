...
# ╔═╡ 4a0aca07-b44f-4ee7-97f8-e3efee1690c6
using RxInfer, StableRNGs, BenchmarkTools, Plots, Dagger

# ╔═╡ 6366a961-e1e9-4085-82fc-aa28f9ab3c9c
module Parallel
	include("../parallel.jl")
end

# ╔═╡ 556c5c04-0b46-45d0-a679-454f46ec47e9
Threads.nthreads()

# ╔═╡ 5bacdeb2-e0f0-4426-befd-bcf667edb095
begin
...
# ╔═╡ 82c86e17-2a65-4b24-a636-8ff95d55ea6a
@model function linear_regression(nr_samples, prod_strategy, pipeline)
	a = randomvar() where { prod_strategy = prod_strategy }
	b = randomvar() where { prod_strategy = prod_strategy }

    a ~ Normal(mean = 0.0, variance = 1.0) where { pipeline = pipeline }
    b ~ Normal(mean = 0.0, variance = 100.0) where { pipeline = pipeline }

    x = datavar(Float64, nr_samples)
    y = datavar(Float64, nr_samples)

    y .~ Normal(mean = a .* x .+ b, variance = 1.0) where { pipeline = pipeline }
end

# ╔═╡ cf22df91-fc40-42f3-a376-40b2211ef710
function run_test(prod_strategy, pipeline)
	return infer(
	    model        = linear_regression(length(x_data), prod_strategy, pipeline),
...
# ╔═╡ c0ec99d8-f780-4a82-9460-771648c57a00
begin
...
# ╔═╡ 42c3a398-303a-4e8d-b4ff-7e6fdedd2303
begin
	plot(2:20, result.free_energy[2:end], title="Free energy", xlabel="Iteration", ylabel="Free energy [nats]", label="normal")
	plot!(2:20, result_parallel.free_energy[2:end], title="Free energy", xlabel="Iteration", ylabel="Free energy [nats]", label="parallel")
end

# ╔═╡ d4c663fe-88f3-4099-a5bb-c9376fbb761c
@benchmark run_test(nothing, nothing)

# ╔═╡ f6e28cbc-1035-4ba1-9601-f1d223e75cdd
...
manifest_format = "2.0"
project_hash = "7b1b5672b157c37117078be7ef02ae6599954b8b"
...
[[deps.Dagger]]
deps = ["DataStructures", "Distributed", "LinearAlgebra", "MacroTools", "MemPool", "PrecompileTools", "Profile", "Random", "Requires", "ScopedValues", "Serialization", "SharedArrays", "SparseArrays", "Statistics", "StatsBase", "TimespanLogging", "UUIDs"]
git-tree-sha1 = "f8152a11409a03436ade96eb106e9970d93acae8"
