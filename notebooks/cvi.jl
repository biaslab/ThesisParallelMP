...
# ╔═╡ 16f9fd42-b52a-11ee-342c-7da05478abb8
using RxInfer, Random, LinearAlgebra, Plots, Optimisers, Plots, StableRNGs, SpecialFunctions, BenchmarkTools

# ╔═╡ 9e348056-7613-442c-982e-33bd985fd782
module Parallel
	include("../parallel.jl")
end

# ╔═╡ 34dd10db-7472-452a-8319-32fd62acdcf2
Threads.nthreads()

# ╔═╡ aa9399be-790d-40dd-8776-0592f4a0b8b4
begin
...
# ╔═╡ 614e7889-56b8-42ac-84f5-bbdc949dfb37
@model function measurement_model(nr_observations, prod_strategy, pipeline)
    ...
    for t in 2:nr_observations
        ...
        z[t] ~ Normal(mean = z[t-1] + 1, precision = τ) where { pipeline = pipeline }

        # specify non-linear observation model
        y[t] ~ Normal(mean = compute_squared_distance(z[t]), precision = γ) where { pipeline = pipeline }
        
    end

end

# ╔═╡ bd40364a-16b0-4a47-ab27-46bb99785d75
begin
...
# ╔═╡ 9d7dd1cf-6a1a-4637-a463-eaffbd43580d
function run_test(prod_strategy, pipeline)
	return infer(
	    model = measurement_model(nr_observations, prod_strategy, pipeline),
	    data = (y = measurements,),
	    iterations = 5,
	    free_energy = true,
...
# ╔═╡ 27631bcf-1d1c-4d61-9ffc-e50cc661b2f7
begin
...
# ╔═╡ c4fc6114-1c58-4fab-8322-1e3b960c1991
begin
	...
	plot!(map(mean, result.posteriors[:z]), label = "estimated location (±2σ)", ribbon = map(x -> 2*std(x), result.posteriors[:z]), fillalpha=0.5, linewidth=3, color = :orange)
	plot!(map(mean, result_parallel.posteriors[:z]), label = "estimated location parallel (±2σ)", ribbon = map(x -> 2*std(x), result_parallel.posteriors[:z]), fillalpha=0.5, linewidth=3, color = :green)
	xlabel!("time [sec]"), ylabel!("location [cm]")
	
	# plot Bethe free energy
	p2 = plot(result.free_energy, linewidth=3, label = "")
	p2 = plot!(result_parallel.free_energy, linewidth=3, label = "")
	xlabel!("iteration"), ylabel!("Bethe free energy [nats]")
	
	plot(p1, p2, size = (1200, 500))
end

# ╔═╡ 5c0a31bd-0ecb-4e30-9e28-d85dd6b785a8
@benchmark run_test(nothing, nothing)

# ╔═╡ ad35d4d4-c3f8-4d53-95d7-754fdfe39e16
...
manifest_format = "2.0"
project_hash = "052d93ef983563bd397393853d65a786dcf3cd09"
