...
# ╔═╡ 3feb1880-b55a-11ee-166c-4b6ca084724e
using RxInfer, Random, LinearAlgebra, Distributions, Plots, StatsPlots, Optimisers, DataFrames, DelimitedFiles, StableRNGs, BenchmarkTools

# ╔═╡ cab650d9-18b3-4b20-b7af-73c809f90a61
module Parallel
	include("../parallel.jl")
end

# ╔═╡ 4e5ba971-96f0-47ac-ad27-ab1018e26f1d
Threads.nthreads()

# ╔═╡ 3cfc4c4f-0cd6-43be-b0cc-6ff33479b659
begin
	# fetch measurements
...
# ╔═╡ 7e7e537b-63cc-461c-80dd-50d9efed6e46
@model function random_walk_model(nr_observations, prod_strategy, pipeline)
    ...
    z = randomvar(nr_observations) where { prod_strategy = prod_strategy }
    ...
    for t in 2:nr_observations
        ...
        z[t] ~ MvNormalMeanPrecision(z[t-1], W) where { pipeline = pipeline }

        # specify non-linear distance observations model
        y[t] ~ MvNormalMeanPrecision(compute_distances(z[t]), R)  where { pipeline = pipeline }
        
    end
end;

# ╔═╡ e88cb25a-1f3d-4cd0-ac68-4f034a918ca2
begin
...
# ╔═╡ f4a030b9-1b6d-4e6d-92e1-f3f54358abf4
function run_test(prod_strategy, pipeline)
	return infer(
	    model = random_walk_model(nr_observations, prod_strategy, pipeline),
...
# ╔═╡ c582d65c-f610-4836-9fb5-8a5ba3367f8a
begin
...
# ╔═╡ c3ad578f-9ef2-4435-b494-1e38bc57e1a3
 begin
	...
	map(posterior -> covellipse!(mean(posterior), cov(posterior), color="red", label="", n_std=2), result.posteriors[:z])
	xlabel!("longitude [m]"), ylabel!("latitude [m]"), title!("Sequential"); p1.series_list[end][:label] = "estimated location ±2σ"
	...
	plot!(position[1,:], position[2,:], label="actual location", linewidth=3, linestyle=:dash, arrow=(:closed, 2.0), aspect_ratio=1.0)
	map(posterior -> covellipse!(mean(posterior), cov(posterior), color="red", label="", n_std=2), result_parallel.posteriors[:z])
	xlabel!("longitude [m]"), ylabel!("latitude [m]"), title!("Parallel"); p2.series_list[end][:label] = "estimated location ±2σ"
	
	plot(p1, p2, size=(1200, 500))
 end

# ╔═╡ b13873f5-05cd-464f-a398-6b45e7022a2d
@benchmark run_test(nothing, nothing)

# ╔═╡ bfd3ef8b-3841-4bc6-affb-33a9957a1f6d
@benchmark run_test(Parallel.CustomProdStrategy(Parallel.custom_prod), Parallel.ThreadsPipelineStage())

# ╔═╡ 5621e16f-4c4e-40e1-95d2-88fb84182ccf
@model function random_walk_model_wishart(nr_observations, prod_strategy, pipeline)
    ...
    for t in 2:nr_observations
        ...
        # specify random walk state transition model
        z[t] ~ MvNormalMeanPrecision(z[t-1], Q) where { pipeline = pipeline }
...
# ╔═╡ 778c3347-44ca-47ac-b49a-fb93a6055531
function run_test_wishart(prod_strategy, pipeline)
	return infer(
	    model = random_walk_model_wishart(nr_observations, prod_strategy, pipelin),
	    data = (y = [distances[t,:] for t in 1:nr_observations],),
	    iterations = 100,
	    free_energy = true,
	    returnvars = (z = KeepLast(),),
...
# ╔═╡ 8ed43a41-4d5f-468b-970e-9df72d3cc0f4
@benchmark run_test(nothing, nothing)

# ╔═╡ 4c6017ac-2ab4-4fd6-9aa0-58e1abf1d5e9
@benchmark run_test(Parallel.CustomProdStrategy(Parallel.custom_prod), Parallel.ThreadsPipelineStage())
...
manifest_format = "2.0"
project_hash = "749b82170d21f07e770aedefb5bb091300a7a0f1"
...
git-tree-sha1 = "cf1784b5f2d67beb58ec8f9e0f55b216c94a3038"
...
git-tree-sha1 = "7e4b33150e52e78db4df20b9b7319722471e6ed1"
...
git-tree-sha1 = "ac493e51882df8275195591b17440cb8b865dbdb"
uuid = "a194aa59-28ba-4574-a09c-4a745416d6e3"
version = "3.13.1"
weakdeps = ["Optimisers", "Requires"]
...
git-tree-sha1 = "36f222e7dc0db9bfe4131320ae0cc64e46b6c0df"
