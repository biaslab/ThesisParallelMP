...
# ╔═╡ 89ad22fa-5666-4593-b559-00594e917db2
module Parallel
	include("../parallel.jl")
end
...
# ╔═╡ 4e0b04fd-2d98-44d5-b369-2ab412cb1804
@model function rotate_ssm(n, x0, A, B, Q, P, prod_strategy, pipeline)
    ...
    for i in 1:n
        x[i] ~ MvNormalMeanCovariance(cA * x_prev, cQ) where { pipeline = pipeline }
        y[i] ~ MvNormalMeanCovariance(cB * x[i], cP) where { pipeline = pipeline }
        x_prev = x[i]
    end

end

# ╔═╡ 0791633f-7e33-454f-a8a0-9b23b5c29a17
prior = Beta(2.0, 7.0)

# ╔═╡ fcbdb026-c9f5-4d0a-aca0-9545414d8e82
function run_test(prod_strategy, pipeline)
	return infer(
	    model = rotate_ssm(length(y), x0, A, B, Q, P, prod_strategy, pipeline),
	    data = (y = y,),
...
begin
	...
	xmarginals_parallel  = result_parallel.posteriors[:x]
	logevidence_parallel = -result_parallel.free_energy
end

# ╔═╡ f6168636-3499-445e-9d38-8bd4929b1277
begin
	px = plot()
	...
	px = plot!(px, getindex.(mean.(xmarginals_parallel), 1), ribbon = getindex.(var.(xmarginals_parallel), 1) .|> sqrt, fillalpha = 0.5, label = "Estimated Signal Parallel (dim-1)", color = :yellow)
	px = plot!(px, getindex.(mean.(xmarginals_parallel), 2), ribbon = getindex.(var.(xmarginals_parallel), 2) .|> sqrt, fillalpha = 0.5, label = "Estimated Signal Parallel (dim-2)", color = :blue)
...
manifest_format = "2.0"
project_hash = "7619666846ac25f362c60da067014d8b22a9b597"
